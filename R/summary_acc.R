#' Summarize an `acc` vector
#'
#' @description
#' Provides a diagnostic overview of an `acc` vector, including axis
#' combinations, frequencies, burst sizes, timing, and value ranges.
#'
#' @param object An `acc` vector.
#' @param ... Ignored.
#'
#' @returns An `acc_summary` object.
#'
#' @export
#'
#' @examples
...
summary.acc <- function(object, ...) {
  ...
  out <- list(n = n, n_na = n_na)
  
  if (n_valid == 0) {
    return(new_acc_summary(out))
  }
  ...
  # Units
  bu_all <- burst_units(x)
  out$burst_units <- unique(na.omit(bu_all))
...
print.acc_summary <- function(x, ...) {
  ...
  cat(format_count(x$n), " acc bursts", na_note, "\n", sep = "")
  ...
  cat("\n")
  labels <- character(0)
  if (length(x$burst_units) > 0) labels <- paste0("[", x$burst_units, "]")
...
#' `acc_summary` object.
#'
#' @param x An `acc_summary` object (returned by [summary.acc()]).
...
new_acc_summary <- function(x) {
  structure(x, class = c("acc_summary", class(x)))
}
